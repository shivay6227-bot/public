...
function findQuestMediaUrl(quest) {
    ...
    const add = (value, priority = 0) => {
        ...
        seen.add(url);
        found.push({ url, priority });
    };

    // Prefer the same artwork/thumbnail style Discord normally exposes for a quest.
    const preferred = [
        quest?.preview?.thumbnail,
        quest?.preview?.thumbnail_url,
        quest?.preview?.image,
        quest?.preview?.image_url,
        quest?.preview?.url,
        cfg?.preview?.thumbnail,
        cfg?.preview?.thumbnail_url,
        cfg?.preview?.image,
        ...
        cfg?.assets?.thumbnail_url,
        cfg?.assets?.hero,
        cfg?.assets?.hero_url,
        cfg?.assets?.image,
        cfg?.application?.cover_image_url,
        cfg?.application?.cover_image,
        quest?.targetedContent?.thumbnail,
        quest?.targetedContent?.thumbnail_url,
        quest?.targetedContent?.image,
        quest?.targetedContent?.image_url,
        quest?.targetedContent?.preview,
    ];
    preferred.forEach((value, index) => add(value, 1000 - index));

    // Discord can change the exact field name between quest types. Recursively
    // search preview/config/targeted content and prefer image/banner/thumbnail URLs.
    const walk = (value, path = '', depth = 0) => {
        ...
        if (typeof value === 'string') {
            ...
            let priority = 0;
            if (/thumbnail|banner|hero|cover|image|artwork|preview/.test(key)) priority += 500;
            if (/video|stream/.test(key)) priority -= 100;
            add(value, priority);
            return;
        }
    ...
    walk(quest?.preview, 'preview');
    walk(cfg, 'config');
    walk(quest?.targetedContent, 'targeted_content');
...
async function buildQuestInfoCard(quest, phase, claimed = 0, failReason = '', progressValue = null) {
    ...
    if (mediaUrl) {
        try {
            // Direct thumbnail/banner only — no Open Quest or extra preview button.
            const gallery = new MediaGalleryBuilder().addItems(
